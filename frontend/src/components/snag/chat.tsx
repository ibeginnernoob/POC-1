    ...
    return (
        <div className="bg-white w-[100%]">
            <div className="bg-white max-w-5xl mx-auto px-6 py-8 flex flex-col gap-10">
                ...
                {response && isGenerationSuccess && (
                    <UserMessage response={response} />
                )}
                {!response && isError && <Message type="error" />}
                {response && isGenerationSuccess && (
                    <Message type="generated" response={response} />
                )}
            </div>
        </div>
    );
...
type MsgType = 'initial' | 'error' | 'generated';
            ...
            <div className="bg-blue-100 max-w-2xl px-4 py-5 rounded-2xl">
                ...
                {type === 'generated' && (
                    <div className="flex flex-col gap-5">
                        <p className="text-sm font-medium">
                            <Markdown>
                                {response?.rectification.ai_recommendation}
                            </Markdown>
                        </p>
                        <div>
                            <p className="text-sm font-medium">
                                Recommendation Reliability:{' '}
                                {response?.summary.recommendation_reliability}
                            </p>
                            <p className="text-sm font-medium">
...
function UserMessage({ response }: { response: GeneratedData | null }) {
    return (
